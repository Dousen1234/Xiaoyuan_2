...
四个电机（ID=1、4 减速比 101；ID=2、3 减速比 81）按 **6 阶段单腿步态**运动，每阶段 `kStepSec`（默认 10s，总时长 60s）：

| 阶段 | 动作 |
|------|------|
| 0 | 四电机回零并保持不动 |
| 1 | ID1/4 不动；ID2 转 -180°，ID3 转 +360° |
| 2 | ID2/3/4 不动；ID1 往复 0° → 60° → -60° → 0° |
| 3 | ID1/4 不动；ID2/3 按与阶段 1 相反方向回零 |
| 4 | ID1/2/3 不动；ID4 往复 0° → 90° → -90° → 0° |
| 5 | 四电机回零并保持不动 |
